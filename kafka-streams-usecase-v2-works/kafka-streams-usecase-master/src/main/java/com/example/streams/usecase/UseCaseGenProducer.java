       ...
       JsonSerializer<Usecase> usecaseSerializer = new JsonSerializer<>();
        ...
        while (true) {
            ...
            for (int i = 0; i < 1000000000; i++) {
                ...
                int lon = random.nextInt(500);
                int lat = random.nextInt(200);
                double p = random.nextGaussian() * 0.25 + 1; // random var from lognormal dist with stddev = 0.25 and mean=1
                double pp = random.nextGaussian() * 0.25 + 1;
                ...
                if (iter % 10 == 0) {
                    ...
                    humi = humi + random.nextInt(5 * 2) - 5;

                }

                String measure = "hallo";


                Usecase usecase = new Usecase(sensorId,lon,lat,temp,humi,p,pp,measure);
